...
execute align xzy run particle dust 1 1 1 1.5 ~.5 ~.5 ~.5 0.4 0.4 0.4 0 32
playsound minecraft:item.ink_sac.use block @s ~ ~ ~

# consome items from player
execute as @s[gamemode=!creative] if score option_consume_dye guris.colouring matches 1 run clear @s minecraft:white_dye 1
